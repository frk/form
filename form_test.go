...
func TestTransform(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			vals: url.Values{},
			dst:  nil, want: nil,
			err: &InvalidArgumentError{nil, "Transform"},
		}, {
			...
			err: &InvalidArgumentError{reflect.TypeOf(struct{}{}), "Transform"},
		}, {
			name: "any dst that is not a pointer to a struct should return error",
			dst:  []int{}, want: []int{},
			err: &InvalidArgumentError{reflect.TypeOf([]int{}), "Transform"},
		}, {
			name: "bool type err",
